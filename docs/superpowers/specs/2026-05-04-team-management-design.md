...
┌─ 팀원 관리 ───────────────────────────────────────────────────────┐
│  ┌──────────────────────────────────────────────────────────────┐ │
│  │ 이름  │ 한글이름 │ 04-30 │ 04-23 │ 04-16 │ 04-09 │  면담  │ │
│  │──────────────────────────────────────────────────────────────│ │
│  │ Kel*  │ 켈      │  ☑   │  ☑   │  ☐   │  ☑   │   3    │ │
│  │ Lulu  │ 룰루    │  ☑   │  ☐   │  ☐   │  ☐   │   1    │ │
│  │ Mia   │ 미아    │  ☐   │  ☐   │  ☐   │  ☐   │   -    │ │
...
- 열 구성: 이름 | 한글이름 | (직전 4 일자 — `recent_dates` 순서대로 4컬럼) | 면담.
- 일자 헤더: `MM-DD` 형태 (e.g. `04-30`).
- 각 출석 셀: 체크박스. `attendance[i].attended` 값으로 초기 상태. onclick → `toggle` API → 실패 시 롤백. 성공 시 그 셀만 즉시 반영(전체 재렌더링 없음).
- 체크박스 셀 클릭은 `stopPropagation` → 행 클릭(상세 이동)과 분리.
- 이름/한글이름/면담 셀 클릭(행 클릭) → `#team/<coach_id>` 이동.
- 면담 카운트 0이면 `-` 표시, 1+ 면 숫자.
- 정렬: 본인 첫 행 → 이후 coach_name ASC.
- 팀원 0명 시 empty-state.
- 모바일: `data-table-wrapper`의 `overflow-x:auto`로 가로 스크롤.
