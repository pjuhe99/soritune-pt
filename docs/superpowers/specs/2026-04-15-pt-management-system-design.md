...
|--------|------|-------------|
| id | INT AUTO_INCREMENT PK | |
| soritune_id | VARCHAR(50) NOT NULL UNIQUE | soritunenglish.com 회원 ID (필수, 중복 불가, 자연 키) |
...
**회원 식별 원칙**: `soritune_id`가 회원의 자연 키다. import/주문 매칭/수동 추가 모두 `soritune_id` 기준으로 수행한다. 동일 인물이 여러 ID로 등록된 경우는 "동일인 관리(merge)"로 해결한다 — 이름+전화 기반 자동 매칭은 사용하지 않는다.

### member_accounts

member_accounts는 병합 이력 및 부계정 보관용이다. 회원 병합 시 피병합 회원의 원본 정보를 이 테이블에 보존한다.
