...
--
-- 적용 (DEV):
--   mysql --defaults-file=/root/.my.cnf SORITUNECOM_DEV_PT < migrations/20260511_add_coaching_chart.sql
-- 적용 (PROD):
--   mysql --defaults-file=/root/.my.cnf SORITUNECOM_PT < migrations/20260511_add_coaching_chart.sql
--
-- 멱등성: CREATE TABLE IF NOT EXISTS / ALTER ... ADD COLUMN IF NOT EXISTS / ADD KEY IF NOT EXISTS.
-- 주의: FK constraint(fk_session_calendar)는 IF NOT EXISTS 미지원 — 재실행 시 에러 발생.
--        재실행 필요 시 사전 수동 DROP: ALTER TABLE order_sessions DROP FOREIGN KEY IF EXISTS fk_session_calendar;
-- 롤백: 수동 (DROP TABLE / DROP COLUMN). 자동 롤백 스크립트 없음.
...
-- 3) order_sessions 확장
--    주의: 재실행 시 fk_session_calendar 에러 발생 (수동 제거 또는 별도 처리)
ALTER TABLE `order_sessions`
  ADD COLUMN IF NOT EXISTS `calendar_id`  INT       NULL AFTER `order_id`,
  ADD COLUMN IF NOT EXISTS `progress`     TEXT      NULL AFTER `memo`,
  ADD COLUMN IF NOT EXISTS `issue`        TEXT      NULL AFTER `progress`,
  ADD COLUMN IF NOT EXISTS `solution`     TEXT      NULL AFTER `issue`,
  ADD COLUMN IF NOT EXISTS `improved`     TINYINT(1) NOT NULL DEFAULT 0 AFTER `solution`,
  ADD COLUMN IF NOT EXISTS `improved_at`  DATETIME  NULL AFTER `improved`,
  ADD COLUMN IF NOT EXISTS `updated_by`   INT       NULL AFTER `improved_at`,
  ADD COLUMN IF NOT EXISTS `updated_at`   DATETIME  NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  ADD KEY IF NOT EXISTS `idx_order_completed` (`order_id`, `completed_at`),
  ADD KEY IF NOT EXISTS `idx_order_improved`  (`order_id`, `improved`),
